...
"! Trees are walked by ZCL_ACE_KEYWORDS=>WALK_NODE, which follows Expression
...
public section.
  ...
  " opening yet another popup. The toolbar toggles it.
  data MV_NEW_WINDOW type ABAP_BOOL .
  ...
  " Opens the clicked unit in the existing source window
  methods SHOW_SOURCE_IN_MAIN
    importing
      !IS_NODE type TS_NODE_MAP .
...
ENDMETHOD.
  ...
  ENDMETHOD.
  METHOD add_unit.
    cs_tot-units = cs_tot-units + 1.
    cs_tot-cc = cs_tot-cc + is_unit-cyclomatic.
    cs_tot-loc = cs_tot-loc + is_unit-loc.
    cs_tot-lloc = cs_tot-lloc + is_unit-lloc.
    cs_tot-cloc = cs_tot-cloc + is_unit-cloc.
    cs_tot-n1 = cs_tot-n1 + is_unit-n1.
    cs_tot-n2 = cs_tot-n2 + is_unit-n2.
...
DATA(lv_maxlen) = 200.
  ...
  method HND_TOOLBAR.
      ...
      IF fcode = 'NEWWIN'.
        mv_new_window = xsdbool( mv_new_window IS INITIAL ).
        mo_toolbar->set_button_info(
          EXPORTING fcode = 'NEWWIN'
                    text  = COND #( WHEN mv_new_window = abap_true
                                    THEN 'New window' ELSE 'Main window' ) ).
        RETURN.
      ENDIF.
  ...
  method ON_NODE_CLICK.
    ...
    CHECK sy-subrc = 0.
    IF mv_new_window = abap_true.
      show_source_popup( ls_node ).
    ELSE.
      show_source_in_main( ls_node ).
    ENDIF.
  endmethod.
  method SHOW_SOURCE_IN_MAIN.
    " Same node resolution as SHOW_SOURCE_POPUP, but the unit is opened in
    " the window the user already has, instead of stacking up popups.
    DATA lv_inc TYPE program.
    ...
    " Line of the unit itself, so the window lands on it rather than on top
    DATA(lv_line) = 1.
    ...
    IF sy-subrc = 0.
      ...
      LOOP AT lr_kw->* INTO DATA(ls_kw) WHERE index = ls_cl-index.
        lv_line = COND i( WHEN ls_kw-v_line > 0 THEN ls_kw-v_line ELSE ls_kw-line ).
        EXIT.
      ENDLOOP.
    ENDIF.

    mo_viewer->mo_window->m_prg-include = lv_inc.
    mo_viewer->mo_window->set_program( lv_inc ).
    mo_viewer->mo_window->set_program_line( lv_line ).
    mo_viewer->mo_window->set_nav_caption( lv_inc ).
    IF mo_viewer->mo_window->mo_box IS NOT INITIAL.
      mo_viewer->mo_window->mo_box->set_focus( mo_viewer->mo_window->mo_box ).
    ENDIF.
  endmethod.
